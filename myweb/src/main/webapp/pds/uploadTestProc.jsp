...
<%
	...
	try {
		...
		//3. /storage폴더에 저장된 파일 정보 확인
		//	1)mr에서 <input type=file>를 전부 수거하기
		//	  열거형 Enumeration enum={"KIM", "LEE", "PARK"}
		//	  만일, 첨부된 파일이 3개 있다면 files={<input type=file>, <input type=file>, <input type=file>, ...}
		Enumeration files = mr.getFileNames(); //자료형 Enumeration 
		...
		//	3) 2)의 item 이름 (filename)이 가지고 있는 실제 파일을 mr객체에서 가져오기
		...
		out.print("<a href='javascript:history.back();'>[다시시도]");
	}
